...
function move_robot(_robot: Robot = new Robot()): number {
    ...
    if (tagsInView.indexOf(_robot.target) >= 0 || HIL_Simulation) {
...
basic.forever(function on_forever() {
    ...
    if (!stopping) {
        if ((robot.cmd == CLAIM_DESTINATION || robot.cmd == RELEASE_ROUTE) && !WaitingForReply) {
            if (DEBUG) {
                if (robot.cmd == CLAIM_DESTINATION) {
        ...
        } else if (WaitingForReply && input.runningTime() - RadioTxTime > 1000) {
